...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Xnuva address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no xnuva: URI
    if(!uri.isValid() || uri.scheme() != QString("xnuva"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("xnuva:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
