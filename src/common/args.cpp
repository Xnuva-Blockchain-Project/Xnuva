...
const char * const BITCOIN_CONF_FILENAME = "xnuva.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Xnuva
    //   new: C:\Users\Username\AppData\Local\Xnuva
    // macOS: ~/Library/Application Support/Xnuva
    // Unix-like: ~/.xnuva
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Xnuva";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Xnuva";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Xnuva";
#else
    // Unix-like
    return pathRet / ".xnuva";
#endif
#endif
}
