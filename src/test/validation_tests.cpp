...
//! XNUV regtest currently publishes no AssumeUTXO snapshots.
BOOST_AUTO_TEST_CASE(test_assumeutxo)
{
    ...
    BOOST_CHECK(
        params->GetAvailableSnapshotHeights().empty());

    for (const int height :
         {0, 100, 110, 111, 115, 209, 211}) {
        BOOST_CHECK(
            !params->AssumeutxoForHeight(height));
    }

    BOOST_CHECK(
        !params->AssumeutxoForBlockhash(
            uint256{
