...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " xnuva-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The xnuva-tx tool is used for creating and modifying Xnuva transactions.\n\n"
                "xnuva-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded Xnuva transaction, or with \"-create [commands]\" to create a hex-encoded Xnuva transaction.\n"
                "\n"
                "Usage: xnuva-tx [options] <hex-tx> [commands]\n"
                "or:    xnuva-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded Xnuva transaction
