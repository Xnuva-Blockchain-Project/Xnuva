...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // Xnuva has no historical duplicate-coinbase exception blocks.
    // BIP30 semantics are enforced for every block.
    constexpr bool fEnforceBIP30 = true;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP30: never allow a transaction to overwrite an existing
    // unspent transaction output. Xnuva is a fresh chain and has no
    // historical duplicate-coinbase exception blocks.
...
bool IsBIP30Repeat(const CBlockIndex&)
{
    return false;
}

bool IsBIP30Unspendable(const uint256&, int)
{
    return false;
}
